...
export default function JoinRoom() {
  ...
  useEffect(() => {
    ...
    api.get(`/v1/rooms/join/${token}`)
      .then((res) => {
        setStatus('success')
        setMessage(`You joined "${res.data.roomName}"! Redirecting...`)
        setTimeout(() => navigate(`/room/${res.data.roomId}`), 2000)
      })
      .catch((err) => {
        setStatus('error')
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <h2 style={styles.title}>
          {status === 'loading' ? 'Joining room...' : status === 'success' ? 'Joined!' : 'Invalid Invite'}
        </h2>
